...
const publicPhotoUrl = (photo, width = 1600) => photo?.key
  ? `https://api.owntravel.ru/thumbnail/${String(photo.key).split("/").map(encodeURIComponent).join("/")}?w=${width}`
  : photo?.url || "";
