...
ПОДПИСИ:
- title — короткий самостоятельный заголовок фотографии: 2–6 слов, обычно именная группа, без точки в конце;
- title не должен быть обрезанным началом text, заканчиваться предлогом, союзом или запятой;
- text дополняет title отдельным наблюдением, а не повторяет его теми же словами;
...
- title каждой scene обязателен; place может быть непустым только при подтверждении в данных ТОЙ ЖЕ фотографии.
