...
async function buildStoryboard(payload, reviewSourceFile, selectedIds) {
  const prompt = `Собери storyboard для авторского тревел-журнала уровня хорошего независимого журнала о путешествиях.

Главный принцип: фотография несёт историю, текст только добавляет то, чего в самой фотографии нет. Это НЕ каталог кадров и НЕ технический отчёт.

ИСТОЧНИКИ:
- утверждённый автором review — только как доказательство того, что видно и какие кадры выбраны;
- author_notes и chapter_context — авторский смысл и маршрут;
- author_feedback — обязательные редакторские замечания автора, имей высший приоритет среди стилистических решений.

ВАЖНО ПРО VISUAL LABELS:
- label/описание кадра из review НЕ является готовой подписью и почти никогда не должно попадать в текст дословно;
- не копируй перечисления объектов из label;
- не используй канцелярские, научные и классификационные слова вроде «членистоногое», «биота», «донные отложения», «акватория», если сам автор не просил об этом и это не проверенный необходимый факт;
- если текст лишь сообщает читателю то, что он и так видит на снимке, оставь text пустой строкой;
- допустимо оставить пустыми text и/или title у части сцен: сильной фотографии не нужна подпись ради подписи.

ЗАДАЧА ТЕКСТА:
Текст может делать только одно из четырёх:
1. добавлять авторский контекст или маршрут;
2. связывать два визуальных эпизода;
3. сообщать короткий проверенный факт, которого не видно на фото;
4. задавать ритм или финальную мысль на основе авторских заметок.
Если ни одна задача не выполняется — текста быть не должно.

Жёсткие правила:
- scenes содержат только hero/story;
- каждый утверждённый hero/story public_id используется ровно один раз;
- backstage/skip в scenes не включать;
- не смешивать разные места ради визуального сходства;
- порядок подтверждённых локаций следует маршруту chapter_context;
- не придумывать чувства автора, события, разговоры, погоду, время суток, историю объектов и точные локации;
- не давать видовую идентификацию животных, если её не подтвердил автор;
- не писать отдельную подпись к каждому кадру автоматически: объединяй близкие по смыслу кадры в сцены, когда это не ломает реальный маршрут;
- финальная сцена автора имеет высокий приоритет, если ей соответствует выбранная фотография.

СТИЛЬ:
Спокойный, умный, лаконичный, журнальный. Ориентир — текст рядом с сильной фотографией, а не экскурсионная справка. Фразы короткие. Без восторженности и литературной мишуры. Не объясняй очевидное.

Запрещено:
- «на фотографии», «в кадре мы видим», «представлен», «изображён»;
- перечисление предметов через запятую как основной текст сцены;
- «захватывающие виды», «жемчужина», «словами не передать», «незабываемый», «обязательно стоит», «живописный уголок», «величие природы»;
- технические описания вида «скалистый берег с низкорослой растительностью и спокойным морем» — это метаданные, не журнальный текст.

Один визуальный блок — одна мысль. Обычно 0–2 коротких предложения. Лучше пустой текст, чем слабый.
