...
test("reuses a valid Google OAuth token from sessionStorage", async t => {
  const values = new Map([[key, JSON.stringify({ access_token: "cached", expires_at: Date.now() + 3_600_000, scope: "scope" })]]);
...
test("renews a cached token that predates scope tracking", async t => {
  const values = new Map([[key, JSON.stringify({ access_token: "old", expires_at: Date.now() + 3_600_000 })]]);
  globalThis.sessionStorage = storage(values);
  globalThis.google = { accounts: { oauth2: { initTokenClient(options) { return { requestAccessToken() { options.callback({ access_token: "fresh", expires_in: 1800, scope: "scope" }); } }; } } } };
  t.after(() => { delete globalThis.google; delete globalThis.sessionStorage; });
