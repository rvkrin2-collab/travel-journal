    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      try {
        ...
        if (constraintError) {
          ...
          if (attempt < 3) {
            attemptPrompt = `${prompt}\n\nВАЖНО: предыдущий ответ был неполным. Исправь его и верни весь результат заново. Локальная проверка: ${constraintError}. Не пропускай элементы из исходного списка.`;
            continue;
          }
