...
function localSchemaConstraintError(value, schema, path = "$") {
  ...
  if (schema.type === "array" && Array.isArray(value)) {
    ...
    const itemRequiresPublicId = schema.items?.type === "object"
      && Array.isArray(schema.items.required)
      && schema.items.required.includes("public_id");
    if (itemRequiresPublicId) {
      const seenPublicIds = new Set();
      for (const item of value) {
        const publicId = String(item?.public_id || "");
        if (publicId && seenPublicIds.has(publicId)) return `${path} contains duplicated public_id: ${publicId}`;
    ...
    for (let attempt = 0; attempt < 4; attempt++) {
      ...
      try {
        ...
        if (constraintError) {
          ...
          if (attempt < 3) {
            attemptPrompt = `${prompt}\n\nВАЖНО: предыдущий ответ не прошёл локальную проверку. Исправь его и верни весь результат заново. Ошибка: ${constraintError}. Каждый public_id из исходного списка должен встретиться ровно один раз; ничего не пропускай, не дублируй и не подменяй.`;
            continue;
          }
