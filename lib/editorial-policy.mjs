...
- story: от ${selection.story_min ?? 7} до ${selection.story_max ?? 10};
...
- одинаковая duplicate_group допустима только при совпадении главного объекта и визуальной функции;
- общий фон никогда сам по себе не создаёт группу дублей;
...
- запрещены также словоформы и близкие конструкции про традиционный уклад, культурный контекст, гармонию с природой и величественность;
...
export function validateEditorialRecommendation(raw, items, policy) {
  ...
  for (const id of ids) {
    ...
    for (const pattern of language.forbidden_patterns || []) {
      if (new RegExp(pattern, "iu").test(text)) errors.push(`запрещённая конструкция для ${id}: ${pattern}`);
    }
  ...
  if (storyIds.length < (selection.story_min ?? 7) || storyIds.length > (selection.story_max ?? 10)) {
    errors.push(`story=${storyIds.length}, требуется ${selection.story_min ?? 7}-${selection.story_max ?? 10}`);
  }
  ...
  if (selection.prefer_distinct_visual_functions) {
    ...
    for (const [group, count] of counts) {
      const subject = group.split("·")[0].trim();
      const allowed = selection.max_story_per_duplicate_group_overrides?.[subject]
        ?? selection.max_story_per_duplicate_group_overrides?.[group]
        ?? selection.max_story_per_duplicate_group
        ?? 1;
