...
const PENDING_STAGE_LABELS = ["Ожидаем фотографии", "Анализируем фотографии", "Готовим ИИ-отбор", "Нужно утвердить фотографии", "Собираем preview", "Нужно утвердить preview"];
...
export function chapterStatus({ photos, analysis, ai, author, storyboard, approval }, now = Date.now()) {
  ...
  let kind = "working", label = nextIndex < 0 ? "Глава полностью утверждена" : PENDING_STAGE_LABELS[nextIndex], instruction = "Страница проверит результат автоматически.", action = "";
...
export function overallStateLabel(states, published = false) {
  if (published) return "Опубликовано";
  if (states.some(state => state.kind === "stalled")) return "Нужна помощь";
  if (states.some(state => state.kind === "action")) return "Ждёт вашего действия";
  if (states.length && states.every(state => state.kind === "done")) return "Готово к публикации";
  return "Обработка идёт";
}
