...
Promise.all([import("./lib/photo-services-config.mjs?v=22"), import("./google-photos-picker.js?v=22")]).then(async ([{ validatePhotoServicesConfig }, { GooglePhotosPicker }]) => { const response = await fetch("./config/photo-services.json", { cache: "no-store" }); if (!response.ok) throw new Error(`config HTTP ${response.status}`); const config = validatePhotoServicesConfig(await response.json()); uploadApiUrl = config.upload_api_url.replace(/\/$/, ""); photoPicker = new GooglePhotosPicker(config); document.querySelectorAll("img[data-failed-key]:not([data-fallback])").forEach(img => { img.dataset.fallback = "worker"; img.src = `${uploadApiUrl}/media/${img.dataset.failedKey.split("/").map(encodeURIComponent).join("/")}`; }); }).catch(error => setNote(`Автоматическая отправка недоступна: ${error.message}`));
