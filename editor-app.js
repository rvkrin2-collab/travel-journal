...
function card(photo, index) {
  ...
  const technical = document.createElement("details"); technical.className = "technical"; const technicalTitle = document.createElement("summary"); technicalTitle.textContent = "Технические данные"; const id = document.createElement("div"); id.className = "id"; id.textContent = photoId(photo); technical.append(technicalTitle, id);
  ...
  info.append(imageState, choices, labelHint, labelInput, noteHint, noteInput, technical); article.append(img, info); return article;
}
...
async function init() {
  ...
  document.querySelector("#contactSheetLink").href = `contact-sheet.html?trip=${encodeURIComponent(trip)}&chapter=${encodeURIComponent(chapter)}`;
...
Promise.all([import("./lib/photo-services-config.mjs?v=24"), import("./google-photos-picker.js?v=24")]).then(async ([{ validatePhotoServicesConfig }, { GooglePhotosPicker }]) => { const response = await fetch("./config/photo-services.json", { cache: "no-store" }); if (!response.ok) throw new Error(`config HTTP ${response.status}`); const config = validatePhotoServicesConfig(await response.json()); uploadApiUrl = config.upload_api_url.replace(/\/$/, ""); photoPicker = new GooglePhotosPicker(config); document.querySelectorAll("img[data-failed-key]:not([data-fallback])").forEach(img => { img.dataset.fallback = "worker"; img.src = `${uploadApiUrl}/media/${img.dataset.failedKey.split("/").map(encodeURIComponent).join("/")}`; }); }).catch(error => setNote(`Автоматическая отправка недоступна: ${error.message}`));
